...
impl Calculator {
    ...
    pub fn show_history(&self) -> String{
        let mut all_opers = String::new();
        for (index,operations) in self.history.iter().enumerate(){
            let result = operations.operation_type.perform(
                operations.first_num, operations.second_num)
                .unwrap();

            let formatted_line = format!(
                "{}: {} {} {} = {}\n",
                index,
                operations.first_num,
                operations.operation_type.get_sign(),
                operations.second_num,
                result

            );
            all_opers.push_str(formatted_line.as_str());
        }
        all_opers
    }
    ...
    pub fn repeat(&mut self, operation_index: usize) -> Option<i64>{
        let operation_validated = self.history.get(operation_index)?.clone();

        let result = operation_validated.operation_type.perform(
            operation_validated.first_num,
            operation_validated.second_num
        );
        self.history.push(operation_validated);
        result
    }
